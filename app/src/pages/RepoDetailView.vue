  ...
  <div class="repo-detail-container">
    ...
    >
      ...
      <div v-else-if="repo">
        ...
        <div class="repo-header">
          <div>
            ...
            >
              ...
              <span
                ...
                {{ repo.dataCriacao }}</span
              >
              <span
                ><i class="fas fa-clock"></i> Último commit:
                {{ repo.ultimoCommit }}</span
              >
            </div>
          </div>
          ...
          <div style="display: flex; gap: var(--space-3)">
            ...
            <Button
              variant="outline"
              @click="$router.push(`/repos/${repo.id}/editar`)"
            >
              ...
              <div class="files-list">
                <div v-for="file in files" :key="file.name" class="file-item">
                  <div class="file-info" @click="handleFileClick(file)">
                    ...
                    ></i>
                    <span class="file-name">{{ file.nome }}</span>
                  </div>
                  <div class="file-meta">
                  ...
                  <div class="file-actions">
                    <button
                      v-if="file instanceof NoArquivo"
              ...
              >
                ...
                <div class="commit-meta">
                  ...
                  <span
                    ...
                    {{ commit.arquivosAlterados }} arquivos alterados</span
                  >
                </div>
      ...
      <div v-else class="empty-state">
        <i
          class="fas fa-folder-open"
          style="font-size: 4rem; color: var(--text-tertiary)"
        ></i>
...
const loadRepo = async () => {
  ...
  try {
    ...
    repo.value = data;
    atualFolder.value = repo.value.branchPrincipal.raiz;
  } catch (error) {
...
const loadFiles = async () => {
  ...
  try {
    atualFolder.value = repo.value.branchPrincipal.raiz;
    atualBranch.value = repo.value.branchPrincipal;
    files.value = atualFolder.value.getFilhos();
    console.log(atualFolder.value);
...
const entrarNaPasta = (pasta) => {
  atualFolder.value = pasta;
  files.value = atualFolder.value.getFilhos();
  currentPath.value = atualFolder.value.getCaminho();
};

const getFileIcon = (file) => {
  if (file instanceof NoDiretorio) return "fas fa-folder";
...
const getFileColor = (file) => {
  if (file instanceof NoDiretorio) return "var(--color-warning)";
  const ext = file.nome.split(".").pop().toLowerCase();
  const colors = {
...
const handleCreateFolder = async () => {
  ...
  try {
    ...
    newFolder.value = { name: "" };
    alert(`Pasta "${newFolderItem.nome}" criada com sucesso!`);
  } catch (error) {
...
const handleCreateFile = async () => {
  ...
  try {
    ...
    const contentSize = newFile.value.content
      ? new Blob([newFile.value.content]).size
      : 0;

    const newFileItem = new NoArquivo(newFile.value.name, atualFolder.value);

    const fileVersion = new Versao(
      contentSize,
      "Arquivo criado",
    ...
    atualFolder.value.criarFilho(newFileItem);
    ...
    alert(`Arquivo "${newFileItem.nome}" criado com sucesso!`);
  } catch (error) {
